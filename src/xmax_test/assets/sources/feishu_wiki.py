...
class FeishuWikiSource(BaseSource):
    ...
    def _resolve_target(self) -> dict[str, Any]:
        ...
        if not obj_type or not obj_token:
            raise ContractError(
                f"wiki node {self._wiki_token} returned no obj_type/obj_token"
            )
    ...
    def list_assets(self, query: dict[str, Any] | None = None) -> list[RemoteAsset]:
        ...
        raise ExternalServiceError(
            f"wiki node {self._wiki_token} has unsupported obj_type "
            f"{target['obj_type']!r}"
        )
    ...
    def _bitable(self) -> "FeishuBitableSource":
    ...
    def _sheet(self) -> "FeishuSheetSource":
...
def _clone_descriptor(descriptor: Any, token: str) -> Any:
    ...
    return type(descriptor)(descriptor.source_id, descriptor.kind, descriptor.enabled, descriptor.asset_kind, config)
