...
XMAX 与 Decart Lucy 2.5 视频模型测试平台。生产生成和评测默认使用 XMAX；只有用户在当前任务中明确指定其他视频模型或 Provider 时，才允许切换。Decart/Lucy 等适配器或测试模板的存在不代表获得真实调用授权。浏览器实时生成、RTC证据和网络准入仍是 XMAX 专有测试项。已实现可追溯、可续跑、可版本化的分阶段核心流程：
...
15. 生产生成与评测默认 `generation_provider=xmax`；除非用户当次明确要求，不得使用 Decart/Lucy 或其他视频模型。
