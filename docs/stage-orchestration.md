...
| `plan` | Asset Batch、Benchmark、Scenario Pack、Recipe Pack | 冻结TestPlan |
...
标准输出引用实体是：`asset_batch`、`test_plan`、`run_batch`、`preprocess_batch`、`evaluation_batch`、`human_signal_batch`、`report_bundle`和`sync_batch`。`ingest`的外部输入快照使用`source_snapshot`。Stage Manifest只保存引用和哈希，不把大文件嵌入JSON。
