...
同一标准中的平行细则可分配给不同Judge。例如P.1由Run Metric验证结果登记、CV验证解码与空画面、MLMM补充错误页面和无意义画面；任一可信Judge判0即可触发Gate。普通计分标准仍先合并同一细则证据，再由Fusion对适用细则取平均。

## 2. 主判矩阵

| 评测标准关注点 | 主判 | 辅助 | 原因 |
| --- | --- | --- | --- |
| P.1单次结果有效性 | Metric + CV | MLLM | 文件登记、可解码与有意义画面缺一不可 |
| P.2/P.3/P.4批次成功、重复稳定性与离线时效 | Batch Reporter | 单条P.1、分离后的离线耗时与成片时长 | 不赋0/1/2，不回灌单视频 |
| Prompt核心目标、对象关系、多阶段完成（E1） | MLLM | 运行事实 | 需要理解语言和视频语义 |
...
| G1/G2基础与时序质量 | CV | 实时帧到达Metric | 坚持可客观检测，不混入AI味 |
| G3音频完整性与同步 | Audio Metric | 专项音画模型 | 来源和时间轴优先；实时SDK请求订阅但远端无音频轨时不适用 |
| 视觉融合、贴图感、可懂性和整体观感 | Codex MLLM | CV质量特征 | 语义与感知综合判断 |
| 源视频非目标区域保持和编辑边界 | CV | Codex MLLM | 对齐后的区域差异更可靠 |
| R1交互响应速度 | Metric | CV检测视觉生效时刻 | 只评价快不快 |
| R2交互准确性 | MLLM | 事件和轨迹事实 | 只评价准不准 |
| RP.1/RP.2/RP.3实时启动、恢复与持续时延 | Batch Reporter | RTC/帧/恢复/连续事件事实及网络Profile | 只进入模型批次报告 |
