...
P.2批次生成成败、P.3同输入重复稳定性、RP.1启动与画面交付、RP.2稳定与恢复不进入单视频分数，但必须在对比报告中逐项出现；当批次不适用或证据不足时明确标状态，不得省略或伪造结论。
...
成功退出前验证报告文件存在、P0/P1/P2章节均出现、所有请求场景均出现、全部Benchmark细则与P.2/P.3/RP.1/RP.2均有结果或状态、审计引用可解析。只有完成Agent针对性分析后，才可将该Markdown当作最终分析交付。
