...
- 限流、5xx和网络错误只在原免费模型有界退避，并遵守`Retry-After`；不能把网络错误当额度错误切模型；
- 仅已验证的免费额度规则可以切换模型，未知quota类响应必须暂停，语义Agent只能报警/建议规则，不能授权付费；
...
4. 429/5xx/网络同模型退避，以及已确认免费额度耗尽后的允许回退；
5. 永久认证失败不无限重试；
6. 返回非JSON、缺字段或越界分数；
7. usage和实际模型名被保存；
8. 日志不包含密钥；
9. 收费候选不在末位时配置加载失败，未授权/超限时任何Judge均不开始，流式生成仍能排空。
10. 未知quota不切模型、不进付费，付费网络失败不自动重试。
