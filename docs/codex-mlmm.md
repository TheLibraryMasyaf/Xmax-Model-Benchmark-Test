...
模型顺序以`config/judges.json`为唯一真源。2026-08-24为避免日期快照别名产生额外超时和重复探测，移除了10个带`YYYY-MM-DD`后缀的候选。当前共16个候选：前15个只使用免费额度，唯一末位兜底是泛化别名`qwen3-vl-flash`。前15个模型必须保持“免费额度用完即停”开启；只有末位`qwen3-vl-flash`在人工充值并准备启用付费时关闭该开关。
