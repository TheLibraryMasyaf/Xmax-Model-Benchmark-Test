...
> 本模板用于报告某个XMAX模型版本在一次指定测试批次中的绝对表现，不与其他版本比较。所有占位符必须由Run、Evaluation、Judgment、Benchmark或人工修订数据填入；没有证据时写“未取得/不可评”，不得猜测。
...
| 维度ID | 评分标准 | 模式 | 维度原始分 | 归一化得分 | 有效权重 | 加权贡献 | 可评Run/总Run | 标准差 | 状态 | 证据 |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |
| `{{ dimension_id }}` | `{{ dimension_name }}` | `{{ mode }}` | `{{ raw_score }}/2` | `{{ score_percent }}` | `{{ effective_weight }}` | `{{ weighted_contribution }}` | `{{ assessable_n }}/{{ total_n }}` | `{{ stdev }}` | `strong / acceptable / weak / unassessable / inapplicable` | `{{ evidence_ids }}` |

维度排名与权重解读：`{{ dimension_analysis }}`

## 5. 评分细则得分

> 每条适用细则占一行，按0（差）/1（合格）/2（好）的Benchmark锚点汇总。新Evaluation的`criterion_results`是唯一评分源，维度分必须与其可评细则均值一致。历史Evaluation缺细则时写“旧口径，需Replay”；禁止把维度分平均拆给各细则，也禁止任何形式的维度分反推。

| 维度 | 细则ID | 细则名称 | 平均原始分 | 归一化得分 | 0 / (0,1) / 1 / (1,2) / 2 | 可评Run/总Run | 覆盖率 | 标准差 | P25/P75 | 典型证据 | 结论 |
| --- | --- | --- | ---: | ---: | --- | ---: | ---: | ---: | --- | --- | --- |
| `{{ dimension_id }}` | `{{ criterion_id }}` | `{{ criterion_name }}` | `{{ mean_raw_score }}/2` | `{{ score_percent }}` | `{{ n0 }}/{{ n0_to_1 }}/{{ n1 }}/{{ n1_to_2 }}/{{ n2 }}` | `{{ assessable_n }}/{{ total_n }}` | `{{ coverage_percent }}` | `{{ stdev }}` | `{{ p25 }}/{{ p75 }}` | `{{ evidence_ids }}` | `{{ conclusion }}` |
...
| 优先级 | 问题 | 受影响维度/细则 | 受影响Run | 证据 | 建议改进 | 验收指标/来源 | 建议复测集 |
| --- | --- | --- | ---: | --- | --- | --- | --- |
| P0 | `{{ issue }}` | `{{ ids }}` | `{{ n }}` | `{{ evidence_ids }}` | `{{ action }}` | `{{ acceptance_criterion_and_source }}` | `{{ retest_scope }}` |
...
| 优先级 | 问题 | 受影响维度/细则 | 受影响Run | 证据 | 建议改进 | 验收指标/来源 | 建议复测集 |
| --- | --- | --- | ---: | --- | --- | --- | --- |
| P1 | `{{ issue }}` | `{{ ids }}` | `{{ n }}` | `{{ evidence_ids }}` | `{{ action }}` | `{{ acceptance_criterion_and_source }}` | `{{ retest_scope }}` |
...
| 优先级 | 问题 | 受影响维度/细则 | 受影响Run | 证据 | 建议改进 | 验收指标/来源 | 建议复测集 |
| --- | --- | --- | ---: | --- | --- | --- | --- |
| P2 | `{{ issue }}` | `{{ ids }}` | `{{ n }}` | `{{ evidence_ids }}` | `{{ action }}` | `{{ acceptance_criterion_and_source }}` | `{{ retest_scope }}` |
